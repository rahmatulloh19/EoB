/* Shared atoms for Embassy of Burgers */

const EobLogo = ({ size = 18, color = "var(--gold)" }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 10, color }}>
    {/* mark: shield + EB monogram */}
    <svg width={size * 1.15} height={size * 1.4} viewBox="0 0 23 28" fill="none" aria-hidden>
    ...
    <div style={{ fontFamily: "var(--serif)", fontSize: size, letterSpacing: "0.02em", lineHeight: 1, color }}>
      <div style={{ fontStyle: "italic", fontWeight: 500 }}>Embassy</div>
      <div style={{ fontSize: size * 0.55, letterSpacing: "0.32em", textTransform: "uppercase", color: "var(--silver)", marginTop: 2, fontFamily: "var(--sans)", fontWeight: 500 }}>
        of Burgers
      </div>
    </div>
  </div>
...
const BurgerIcon = ({ size = 56, color = "rgba(212,175,55,0.55)" }) => (
  <svg width={size} height={size * 0.78} viewBox="0 0 80 62" fill="none" aria-hidden>
    {/* top bun */}
...
const ImagePlaceholder = ({ height = 260, caption = "BURGER PHOTO", iconSize = 64, flag, country, style = {} }) => (
...
const SectionEyebrow = ({ children, align = "left" }) => (
...
const SectionTitle = ({ children, accent, align = "left", size = 56 }) => (
  <h2 style={{
...
const StarRating = ({ value = 5, size = 12, color = "var(--gold)" }) => (
...
// Burger data shared across pages
const TOP_BURGERS = [
...
Object.assign(window, {
  EobLogo, BurgerIcon, ImagePlaceholder, SectionEyebrow, SectionTitle, StarRating,
  TOP_BURGERS, FULL_MENU,
});
